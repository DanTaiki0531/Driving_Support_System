...
save_dir = "/usr/src/ultralytics/prg/Driving_Support_System/results"
save_frame_dir = "/usr/src/ultralytics/prg/Driving_Support_System/frame_results"
save_frame_boxs_dir = "/usr/src/ultralytics/prg/Driving_Support_System/frame_results_boxs"
...
while cap.isOpened():
    ...
    # フレームを一時的に保存
    frame_path = f"{save_frame_dir}/frame_{frame_count}.jpg"
    cv2.imwrite(frame_path, frame) # (保存するファイル,)
    # フレームカウントを増やす
    frame_count += 1
    ...
    #軸を非表示にする
    plt.axis('off')
    plt.savefig(f"{save_frame_boxs_dir}/{basename}_boxed.jpg", bbox_inches='tight', pad_inches=0.0)
    plt.close(fig)"""
...
# フレームごとにCSVファイルに追記
df.to_csv(f"{save_dir}/results.csv", mode='a', header=not os.path.exists(f"{save_dir}/results.csv"), index=False)
...
for box in boxes:
    ...
    ax.add_patch(rect)
plt.axis('off')
plt.savefig(f"{save_frame_dir}/frame_{frame_count}_boxed.jpg", bbox_inches='tight', pad_inches=0.0)
plt.close(fig)"""
